...
class Hotel(models.Model):
    ...
    category = models.CharField(max_length=4, choices=HOTEL_CATEGORIES)
    waiters = models.IntegerField()
    capacity = models.IntegerField()

    def __str__(self):
        return f'{self.number}. {self.category} with {self.waiters} waiters {self.capacity}people'
...
class Booking(models.Model):
    ...
    def __str__(self):
        return f'{self.user}has{self.hotel}from{self.check_in}to{self.check_out}'
...
class User(models.Model):
    ...
    def __str__(self):
        return f'{self.id}.{self.fname}.{self.lname}.{self.email}.{self.password}.{self.repassword}'
